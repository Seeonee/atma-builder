jQuery(document).ready(function ($) {
    ...
    var db_post = function(data, callback=console.log) {
        var url = dbManager.get();
        if (!url) {
    ...
    // callback: will be passed the response object
    db_create = function(type, name, counts, callback) {
        db_post({
            'action': 'create', 
            'type': type, 
            'name': name, 
    ...
    // name: the name of the entity to look up
    // callback: will be passed the response object
    db_query = function(type, name, callback) {
        db_post({
            'action': 'query', 
            'type': type, 
            'name': name
    ...
    //  }
    // callback: will be passed the response object
    db_update = function(type, name, items, callback) {
        db_post({
            'action': 'update', 
            'type': type, 
            'name': name, 
